...
class App:
    def __init__(self):
        ...
        if logo_exists:
            ...
            wget.download(os_spec.paths.logo_url, os_spec.paths.logo_loc)
        ...
        # Attributes #
        root.title("Scout")
        icon = PhotoImage(file=os_spec.paths.logo_loc)
        ...
        filemenu.add_command(label="Cut", accelerator="Control+X",
                             command=lambda: root.focus_get().event_generate('<<Cut>>'))

        filemenu.add_command(label="Copy", accelerator="Control+C",
                             command=lambda: root.focus_get().event_generate('<<Copy>>'))

        filemenu.add_command(label="Paste", accelerator="Control+V",
                             command=lambda: root.focus_get().event_generate('<<Paste>>'))
        filemenu.add_command(label="Select All", accelerator="Control+A",
                             command=lambda: root.focus_get().select_range(0, 'end'))
    ...
    def downloadButton_command(self):
        try:
            ...
            if self.urlfield.get() == "":
                self.logfield.insert(
                    INSERT, f'ERROR: URL field is empty and cannot be parsed')
        ...
        if self.videoBool and self.audioBool:  # Video and Audio
            try:
                ...
                print()
                self.logfield.insert(
                    INSERT, f'INFO: vcodec="avc1.64001e", res="highest", fps="best", format="video/mp4"')

            except VideoPrivate:
                self.logfield.insert(
                    INSERT, f'ERROR: This video is privated, you can\'t download it\n')
            except VideoRegionBlocked:
                self.logfield.insert(
                    INSERT, f'ERROR: This video is block in your region\n')
            except RegexMatchError:
                self.logfield.insert(
                    INSERT, f'ERROR: Invalid link formatting\n')
            except RecordingUnavailable:
                self.logfield.insert(
                    INSERT, f'ERROR: This recording is unavalilable\n')
            except MembersOnly:
                self.logfield.insert(
                    INSERT, f'ERROR: This video is for channel members only.\nRefer here for more info: https://support.google.com/youtube/answer/7544492\n')
            except LiveStreamError:
                self.logfield.insert(
                    INSERT, f'ERROR: This is a livestream, and not a downloadable video\n')
            except HTMLParseError:
                self.logfield.insert(
                    INSERT, f'ERROR: HTML parsing or extraction has failed')
            except VideoUnavailable:
                self.logfield.insert(
                    INSERT, f'ERROR: This video is unavalilable, may possibly be payed material or region-locked\n')
            ...
            try:
                ...
                self.logfield.insert(
                    INSERT, f'INFO: vcodec="avc1.64001e", format="audio/mp4"')
            ...
            if self.enablePrompts:
                messagebox.showwarning(
                    "Warning", "Video resolutions for this option are lower quailty.")
                self.logfield.insert(
                    INSERT, f'INFO: As of now videos downloaded without audio are fixed to 360p\n')
            try:
                yt = YouTube(query)
                query = self.urlfield.get()

                silent_audioDown = yt.streams.filter(
                    only_video=True).get_by_itag(itag=134)
                silent_audioDown.download(self.path, filename_prefix="Scout_")
                self.logfield.insert(
                    INSERT, f'INFO: vcodec="avc1.4d401e", res_LOW="360p", fps="24fps", format="video/mp4"')
            except VideoPrivate:
                self.logfield.insert(
                    INSERT, f'ERROR: This video is privated, you can\'t download it\n')
            except VideoRegionBlocked:
            ...
            if self.enablePrompts:  # hasnt selected video nor audio
                self.logfield.insert(
                    INSERT, f'ERROR: You can\'t download a video with video or audio\n')
    ...
    def videoFetch(self, yt, query):  # Basic video basic report (used in all download types)
        ...
        # quickly disbaled user ability to edit log
        self.logfield["state"] = "disabled"
    ...
    def browseButton_command(self):
        ...
        if self.changedDefaultDir:
            ...
            askdirectory(initialdir=os_spec.paths.desktop)
    ...
    def helpButton_command(self):
        webbrowser.open(os_spec.paths.repo_url)

    def clearConsole_command(self):
        os_spec.functions.clearConsole_command(self)
